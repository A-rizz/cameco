...
class EmployeeSeeder extends Seeder
{
     ...
     *
     * KEY FIXES:
     * 1. All position lookups now scope by BOTH title AND department_id, so
     *    "Production Manager" in PROD is a different record from the one in RM1.
     * 2. Removed the duplicate $prodManager / $prodWorker declarations that were
     *    silently overwriting the correctly-scoped variables.
     */
    public function run(): void
    {
        ...
        // Ensure Rolling Mill sub-departments exist under Production
        ...
        // ── Helper: fetch position scoped to its own department ───────────
        // This prevents positions with the same title in different departments
        // from resolving to the wrong record.
        $pos = fn (string $title, ?Department $dept): ?Position =>
            $dept ? Position::where('title', $title)->where('department_id', $dept->id)->first() : null;

        // ── Positions per department (scoped lookups) ─────────────────────
        $hrManager      = $pos('HR Manager',           $hr);
        $hrSpecialist   = $pos('HR Specialist',        $hr);

        $itManager      = $pos('IT Manager',           $it);
        $softwareDev    = $pos('Software Developer',   $it);

        $financeManager = $pos('Finance Manager',      $finance);
        $accountant     = $pos('Accountant',           $finance);

        $opsManager     = $pos('Operations Manager',   $operations);

        $salesManager   = $pos('Sales Manager',        $sales);
        $salesRep       = $pos('Sales Representative', $sales);

        // Production dept positions
        $prodManager    = $pos('Production Manager',   $production);
        $prodWorker     = $pos('Production Worker',    $production);

        // Rolling Mill positions — each sub-dept has its own records
        $rm1Worker  = $pos('Production Worker',    $rollingMill1);
        $rm2Worker  = $pos('Production Worker',    $rollingMill2);
        $rm3Worker  = $pos('Production Worker',    $rollingMill3);
        ...
        // EMP-0004 (Software Dev) reports to EMP-0003 (IT Manager)
        if (isset($createdEmployees['EMP-2024-0004'], $createdEmployees['EMP-2024-0003'])) {
            $createdEmployees['EMP-2024-0004']->update([
                'immediate_supervisor_id' => $createdEmployees['EMP-2024-0003']->id,
            ]);
        }

        // EMP-0006 (Accountant) reports to EMP-0005 (Finance Manager)
        if (isset($createdEmployees['EMP-2024-0006'], $createdEmployees['EMP-2024-0005'])) {
            $createdEmployees['EMP-2024-0006']->update([
                'immediate_supervisor_id' => $createdEmployees['EMP-2024-0005']->id,
            ]);
        }

        // EMP-0009 (Sales Rep) reports to EMP-0008 (Sales Manager)
        if (isset($createdEmployees['EMP-2024-0009'], $createdEmployees['EMP-2024-0008'])) {
            $createdEmployees['EMP-2024-0009']->update([
                'immediate_supervisor_id' => $createdEmployees['EMP-2024-0008']->id,
            ]);
        }
